...
def get_policy_func(arg, file_row):
    ...
    def create_role_changes():
        try:
            if file_row['userIdentity_sessionContext_sessionIssuer_userName'] is not float('nan'):
                row_role = file_row['userIdentity_sessionContext_sessionIssuer_userName']
        except:
            row_role = file_row['requestParameters_bucketName']
# TODO: solve nan values in data
        timing = file_row['eventTime']
        ...
        return timing, TargetName_parameter, TargetType_parameter, Operation_parameter, readOnly_parameter, row_role

    def get_permission_changes():

        anomaly = 0
        if file_row['requestParameters_key'] != 0 and 'leak' in file_row['requestParameters_key']:
            anomaly = 1

        filtered_df = permission_file[(permission_file['requestParameters_roleName'] == role)
                                      & (permission_file['eventTime'] < time)]
...
if __name__ == '__main__':
    ...
    for name in runs:
        # Files as objects
        permission_file = pd.read_csv("data/" + app + "/" + experiment_date + "/" + name + "/permission_changes_logs.csv")
        api_file = pd.read_csv("data/" + app + "/" + experiment_date + "/" + name + "/storage_API_calls.csv")
# TODO: check VOD anomaly
        # Arrange files
        ...
        # Dataframe columns
        policy_headers = ['SampleTime', 'RoleName', 'TargetName', 'TargetType', 'PolicyName', 'Resource', 'Operations', 'Effect',
                          'OperationUsed', 'ReadOnly', 'change_s3', 'change_dynamoDB', 'remove_policy']

        headers = ['SampleTime', 'RoleName', 'TargetName', 'TargetType', 'OperationUsed', 'ReadOnly',
                   'change_s3', 'change_dynamoDB', 'remove_policy', 'Anomaly']

        # Create a dataframe from the data pf the response gathered
        df_roles = pd.DataFrame(data=data, columns=headers)
        ...
        df_roles[headers].copy().to_csv("relations_" + app + "_" + experiment_date + "_" + name + ".csv", index=False)
